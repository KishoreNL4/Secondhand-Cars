...
const View = () => {
  ...
  const [selectedImage, setSelectedImage] = useState(
    daata.imageUrl && daata.imageUrl.length > 0
      ? daata.imageUrl[0] // Set the first image as the default
      : ""
  );
  const toggleChat = () => {
    if (isUserLoggedIn) {
  ...
  useEffect(() => {
    if (daata.imageUrl && daata.imageUrl.length > 0) {
      setSelectedImage(daata.imageUrl[0]);
    }
  }, [daata.imageUrl]);
      ...
      <div className="attai">
        <div style={{ flexDirection: "column" }}>
          <div className="monimg">
            <img className="monster" src={selectedImage} alt="" />
          </div>
