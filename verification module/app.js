...
app.get("/message", function (req, res) {
  var data = {
    ...
    MessageType: "M01",
    MessageNo: "LGRQ001",
    MessageDes: res.__("appMessage").UnableToReachServer,
  };
...
app.get("/getDistricts/:StateName", async function (req, res) {
  try {
    ...
    DataRequest.push(
      db
        .collection("TbDistrict")
        .find({ StateName: StateName })
        .sort({ DistrictName: 1 })
        .toArray()
    );
...
app.get("/getSubDistricts/:DistrictName", async function (req, res) {
  try {
    ...
    DataRequest.push(
      db
        .collection("TbSubDistrict")
        .find({ DistrictName: DistrictName })
...
app.get("/getVillages/:SubDistrictName", async function (req, res) {
  try {
    ...
    DataRequest.push(
      db
        .collection("TbVillage")
        .find({ SubDistrictName: SubDistrictName })
        .sort({ VillageName: 1 })
        .toArray()
    );
    Promise.all(DataRequest)
